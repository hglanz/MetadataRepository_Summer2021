...
for(i in 1:nrow(mdf)) {
  tryCatch(expr = {
    mdf[i, "Last.Updated"] <- mdy(mdf[i, "Last.Updated"])
  }, error = function(error) {
    split <- str_split(mdf$Last.Updated, ", ")
    for(j in 1:length(split)) {
      if(length(list[[j]]) > 1) {
        date <- paste(list[[j]][1], list[[j]][2], collapse = ", ")
        mdf[j, "Last.Updated"] <- mdy(date)
      }
    }
  })
}
...
for(i in 1:nrow(mdf)) {
  if(!is.na(mdf[i, "Last.Updated"])) {
    tryCatch(expr = {
      mdf[i, "Last.Updated"] <- mdy(mdf[i, "Last.Updated"])
    }, warning = function(warning) {
      split <- str_split(mdf[i, "Last.Updated"], ", ") %>% pluck(1)
      split <- split[1:3]
      mdf[i, "Last.Updated"] <- split[1:2] %>% paste(collapse = " ") %>% mdy()
    })
  }
  if(!is.na(mdf[i, "Created"])) {
    tryCatch(expr = {
      mdf[i, "Created"] <- mdy(mdf[i, "Created"])
    }, warning = function(warning) {
      split <- str_split(mdf[i, "Created"], ", ") %>% pluck(1)
      split <- split[1:3]
      mdf[i, "Created"] <- split[1:2] %>% paste(collapse = " ") %>% mdy()
    })
  }
}
mdf["Last.Updated"]
POSIXct(mdf$Last.Updated)
```
