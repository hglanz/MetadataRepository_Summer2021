...
mdf <- data.frame(matrix(ncol = length(names(cdph))+length(names(corgis))+length(names(datagov))+length(names(datahub))+length(names(uci))+length(names(zenodo))+length(names(harvard))+length(names(re3data))+length(names(datashare)), nrow = 0))
names(mdf) <- c(names(corgis), names(cdph), names(datagov), names(datahub), names(uci), names(zenodo), names(harvard), names(re3data), names(datashare))
dataframes <- c(corgis, cdph, datagov, datahub, uci, zenodo, harvard, re3data, datashare)
mdf <- rbind(mdf, target(corgis, names(mdf)), target(cdph, names(mdf)), target(datagov, names(mdf)), target(datahub, names(mdf)), target(uci, names(mdf)), target(zenodo, names(mdf)), target(harvard, names(mdf)), target(re3data, names(mdf)), target(datashare, names(mdf)))
