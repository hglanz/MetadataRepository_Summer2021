...
```{r}
# Iterate Figshare
link <- Frost2021Package::links("https://figshare.com/search", ".PzqLM")
full_df <- data.frame(matrix(ncol = 8, nrow = 0))
for(i in link) {
...
mdf <- data.frame(matrix(ncol = length(names(cdph))+length(names(corgis))+length(names(datagov))+length(names(datahub))+length(names(uci))+length(names(zenodo))+length(names(harvard))+length(names(re3data)), nrow = 0))
names(mdf) <- c(names(corgis), names(cdph), names(datagov), names(datahub), names(uci), names(zenodo), names(harvard), names(re3data))
dataframes <- c(corgis, cdph, datagov, datahub, uci, zenodo, harvard, re3data)
mdf <- rbind(mdf, target(corgis, names(mdf)), target(cdph, names(mdf)), target(datagov, names(mdf)), target(datahub, names(mdf)), target(uci, names(mdf)), target(zenodo, names(mdf)), target(harvard, names(mdf)), target(re3data, names(mdf)))
...
#mdf$num_authors <- str_split(mdf$Authors, ", ") %>%
#  str_split(" and ") %>%
#  str_split("; ") %>%
#  sapply(length)
for(i in 1:nrow(mdf)) {
