...
```{r}
# Iterate Figshare
link <- Frost2021Package::links("https://figshare.com/browse", "a")
full_df <- data.frame(matrix(ncol = 8, nrow = 0))
for(i in link) {
...
# Merge keywords
keywords <- c(datagov$Tags, cdph$Tags, corgis$tags, datagov$Topics, harvard$Keyword)
mdf <- data.frame(matrix(ncol = length(names(cdph))+length(names(corgis))+length(names(datagov))+length(names(datahub))+length(names(uci))+length(names(zenodo))+length(names(harvard)), nrow = 0))
names(mdf) <- c(names(corgis), names(cdph), names(datagov), names(datahub), names(uci), names(zenodo), names(harvard))
dataframes <- c(corgis, cdph, datagov, datahub, uci, zenodo, harvard)
mdf <- rbind(mdf, target(corgis, names(mdf)), target(cdph, names(mdf)), target(datagov, names(mdf)), target(datahub, names(mdf)), target(uci, names(mdf)), target(zenodo, names(mdf)), target(harvard, names(mdf)))
...
cross <- crossing(var1 = 1:ncol(mdf), var2 = 1:ncol(mdf))
cross <- mutate(cross, score = map2_dbl(cross$var1, cross$var2, ~similarity_score(.x, .y)))
cross <- mutate(cross, name1 = names(mdf)[var1], name2 = names(mdf)[var2])
cross %>%
  filter(score >= 0.5 & name1 != name2, !is.numeric(names(mdf)[var1]) & !is.numeric(names(mdf)[var2]))
#map_mdf <- map2_dfr(cross$name1[!is.numeric(cross$name1)] %>% data.frame() %>% distinct() %>% c(), cross$name2[!is.numeric(cross$name2)] %>% data.frame() %>% distinct() %>% c(), ~merge_and_drop(mdf, .x, .y))
...
vec <- c()
for(i in 1:nrow(mdf)) {
  vec <- append(vec, str_split(mdf[i, "Downloads"], pattern = ", ") %>%
    pluck(1) %>%
    parse_number() %>%
    sum(na.rm = TRUE))
}
...
vec <- c()
for(i in 1:nrow(mdf)) {
  vec <- append(vec, str_split(mdf[i, "Observations"], pattern = ", ") %>%
    pluck(1) %>%
    parse_number() %>%
    sum(na.rm = TRUE))
}
mdf$Observations <- vec
vec <- c()
for(i in 1:nrow(mdf)) {
  vec <- append(vec, str_split(mdf[i, "Variables"], pattern = ", ") %>%
    pluck(1) %>%
    parse_number() %>%
    sum(na.rm = TRUE))
}
