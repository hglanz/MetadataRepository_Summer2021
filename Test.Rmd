...
df <- mutate(df, FileInfo <- file_info)
df[, c("Name", "Authors", "Unique views", "Unique downloads", "Publication date:", "Keyword(s):", "data", "Communities:")]

scrape_zenodo("https://zenodo.org/record/5081153#.YOZkbi1h23U")
read.csv("./Data/dryad_scraped.csv")
```
```{r}
...
url <- "https://zenodo.org/search?page=1&size=20"
pg <- read_html(url)
links <- html_attr(html_nodes(pg, "h4 .ng-binding"), "href")
```

```{r}
# Merge things
corgis <- read.csv(".Data/CORGIS_metadata.csv")
datagov <- read.csv("./Data/datagov_scraped.csv")
```
