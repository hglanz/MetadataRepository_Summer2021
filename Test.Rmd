...
dryad <- read.csv("./Data/dryad_scraped.csv")
...
```{r}
url <- "https://datadryad.org/stash/dataset/doi:10.5061/dryad.18931zcss"
...
```{r}
dataframe <- scrape_dryad("https://datadryad.org/stash/dataset/doi:10.5061/dryad.mkkwh70xh")
...
```{r}
url <- "https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/1Y0Z06"
name <- scrape_rvest(url, "#title")
  ...
  sz <- c()
  tryCatch(expr = {
    nfiles <- scrape_rvest(url, ".highlightBold")
    nfiles <- nfiles[str_detect(nfiles, "File")]
    nfiles <- str_remove_all(nfiles, "[:Alpha:]") %>%
      str_trim()
    split <- nfiles %>%
      strsplit(" +") %>%
      pluck(1) %>%
      length()
    if(split != 1) {
      nfiles <- strsplit(nfiles, " +")[[1]][3] %>%
      parse_number()
    }else{
      nfiles <- parse_number(nfiles)
    }
    if(nfiles == 1) {
      nfiles <- 2
    }
  ...
  seq <- seq[seq >= 0]
  for(i in seq) {
    name <- scrape_rvest(url, paste("#datasetForm\\:tabView\\:filesTable\\:", i, "\\:fileInfoInclude-filesTable a", sep = ""))
    size <- scrape_rvest(url, paste("#datasetForm\\:tabView\\:filesTable\\:", i, "\\:fileSize", sep = ""))
    size <- checkNull(size)
    if(!is.na(size)) {
      size <- substr(size, 3, str_length(size)) %>% convert()
    }
    fn <- append(fn, name)
    if(!is.na(size)) {
      sz <- append(sz, as.character(size))
    }else{
      sz <- append(sz, size)
    }
  }
  df$FileName <- paste(fn, collapse = ", ")
  df$sizeMB <- paste(sz, collapse = ", ")
  target <- c("Name", "Description", "Subject", "Keyword", "Deposit Date", "Author", "Depositor", "Citation", "FileName", "sizeMB", "Downloads", "Variables", "Observations")
...
df[target]
scrape_datagov("https://catalog.data.gov/dataset/high-operational-temperature-mwir-detectors-with-optical-concentrators")
```
...
```{r}
# Zenodo
url <- "https://zenodo.org/record/5335900#.YS0kZtNKjt1"
...
#df <- dplyr::mutate(df, FileName = paste(file_info$Name, collapse = ", "), SizeMB = paste(file_info$SizeMB, collapse = ", "))
df$FileName <- paste(file_info$Name, collapse = ", ")
df$SizeMB <- paste(file_info$SizeMB, collapse = ", ")
...
for(i in 1:1) {
  ...
  link <- Frost2021Package::links(url, "h4 a")
  link <- link[str_detect(link, "/record/")]
  link <- paste("https://zenodo.org", link, sep = "")
  for(j in link) {
...
datashare$Author <- str_remove_all(string = datashare$Author, pattern = "Data ")

mdf <- build_mdf(list(corgis, cdph, datagov, datahub, uci, zenodo, harvard, re3data, datashare, dryad))
...
cat("\014")
mdf$Downloads <- parse_number(mdf$Downloads)
mdf$Last.Updated <- parse_date(mdf$Last.Updated)
mdf$Created <- parse_date(mdf$Created)
mdf$Last.Updated - mdf$Created
mdf %>%
  filter(str_length(file_size) != 1) %>%
  select(file_size)
```
...
mdf <- merge_and_drop(mdf, "FileName", "filename")
mdf$Tags <- str_remove_all(mdf$Tags, "\"")
```
...
```{r}
mdf <- read.csv("./Data/mdf.csv")
#rdryad::dryad_dataset(dois = c("10.5061/dryad.c866t1g64"))
...
vec <- c()
for(i in link) {
  vec <- append(vec, checkNull(scrape_rvest(i, ".col-xs-2:nth-child(5)")))
}
datahub$`X.Updated.` <- vec
for(i in 1:nrow(datahub)) {
  if(str_detect(datahub[i, "X.Created."], "days")) {
    datahub[i, "X.Created."] <- lubridate::now() - days(parse_number(datahub[i, "X.Created."]))
  }else if(str_detect(datahub[i, "X.Created."], "months")) {
    datahub[i, "X.Created."] <- lubridate::now() - months(parse_number(datahub[i, "X.Created."]))
  }else{
    datahub[i, "X.Created."] <- lubridate::now() - years(parse_number(datahub[i, "X.Created."]))
  }
  
  if(str_detect(datahub[i, "X.Updated."], "days")) {
    datahub[i, "X.Updated."] <- lubridate::now() - days(parse_number(datahub[i, "X.Updated."]))
  }else if(str_detect(datahub[i, "X.Updated."], "months")) {
    datahub[i, "X.Updated."] <- lubridate::now() - months(parse_number(datahub[i, "X.Updated."]))
  }else{
    datahub[i, "X.Updated."] <- lubridate::now() - years(parse_number(datahub[i, "X.Updated."]))
  }
}
datahub$X.Updated. <- date_decimal(parse_number(datahub$X.Updated.))
datahub$X.Created. <- date_decimal(parse_number(datahub$X.Created.))
datahub
```
...
mdf <- read.csv("./Data/mdf.csv")
dryad$has_API <- TRUE
datahub$has_API <- TRUE
harvard$has_API <- TRUE
corgis$has_API <- FALSE
uci$has_API <- FALSE
re3data$has_API <- TRUE
zenodo$has_API <- TRUE
datashare$has_API <- FALSE
datagov$has_API <- TRUE
cdph$has_API <- TRUE

api <- c(dryad$has_API, datahub$has_API, harvard$has_API, corgis$has_API, uci$has_API, re3data$has_API, zenodo$has_API, datashare$has_API, cdph$has_API, datagov$has_API)
mdf$has_API <- api

vec <- c()
str <- str_split(mdf$FileName, ", ")
for(i in 1:nrow(mdf)) {
  if(!is.na(mdf[i, "FileName"])) {
    vec <- append(vec, str %>% pluck(i) %>% length())
  }else{
    vec <- append(vec, NA)
  }
}
mdf$num_files <- vec
mean(mdf$num_files, na.rm = TRUE)
mdf <- merge_and_drop(mdf, "num_files", "X.Files.")
mdf <- mdf[names(mdf) != "X"]
mdf %>%
  filter(str_detect(FileName, "json"))
View(mdf)

mdf$ntags <- map_int(mdf$Tags, ~str_split(.x, ",") %>% pluck(1) %>% length())
```
...
delimiter <- function(x) {
  ...
  if(!is.na(x)) {
    ...
    if(str_detect(x, "zip")) {
      if(length(string) == 0) {
        string <- paste(string, "pipe", sep = "")
      }else{
        string <- paste(string, "pipe", sep = "; ")
      }
    }
    if(str_detect(x, "pdf") | str_detect(x, "tsv")) {
      if(length(string) == 0) {
        string <- paste(string, "\\t", sep = "")
      }else{
        string <- paste(string, "\\t", sep = "; ")
...
delim <- map_chr(mdf$FileName, delimiter)
mdf$delim.1 <- delim
mdf %>%
  select(FileName, delimiter, delim.1) %>%
  filter(!is.na(FileName), !is.na(delim.1))
mdf <- merge_and_drop(mdf, "delimiter", "delim.1")
mdf <- mdf[names(mdf) != "X" & names(mdf) != "X.1"]
```

```{r}
url <- "https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/MRZGNQ"
mdf <- read.csv("./Data/mdf.csv")
scrape_harvard("https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/KF1LAK")


scrape_rvest(url, ".fileNameOriginal a") %>%
  paste(collapse = ", ")
scrape_rvest(url, ".visible-lg-inline") %>%
  parse_number() %>%
  paste(collapse = ", ")
scrape_rvest(url, "#datasetForm\:tabView\:filesTable\:1\:fileSize , #datasetForm\:tabView\:filesTable\:0\:fileSize") %>%
  parse_number()
scrape_rvest(url, "#datasetForm\\:tabView\\:filesTable\\:0\\:fileSize") %>%
  substr(3, str_length(.))
nfiles <- scrape_rvest("https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/MRZGNQ", ".highlightBold")
nfiles <- nfiles[str_detect(nfiles, "of") & str_detect(nfiles, "Files")]
nfiles <- str_remove_all(nfiles, "[:Alpha:]") %>%
  str_trim()
nfiles <- strsplit(nfiles, " +")[[1]][3] %>%
  parse_number()
...
for(i in 1:11986) {
  ...
  for(j in links) {
    print(count)
    print(j)
    nfiles <- scrape_rvest(j, ".highlightBold")
  nfiles <- nfiles[str_detect(nfiles, "of") & str_detect(nfiles, "Files")]
  nfiles <- str_remove_all(nfiles, "[:Alpha:]") %>%
    str_trim()
  nfiles <- strsplit(nfiles, " +")[[1]][3] %>%
    parse_number()
    for(k in 0:nfiles-1) {
      fileSize <- append(fileSize, scrape_rvest(j, paste("#datasetForm\\:tabView\\:filesTable\\:", k, "\\:fileSize", sep = "")) %>% substr(3, length(.)) %>% convert())
    }
    count <- count+1
  }
}
...
mdf <- read.csv("./Data/mdf.csv")
for(i in 1:nrow(mdf)) {
  split <- str_split(mdf[i, "Last.Updated"], ", ") %>% pluck(1)
  if(length(split) != 1) {
    split <- split[1:2] %>% paste(collapse = ", ")
  }
  mdf[i, "Last.Updated"] <- split
  split <- str_split(mdf[i, "Created"], ", ") %>% pluck(1)
  if(length(split) != 1) {
    split <- split[1:2] %>% paste(collapse = ", ")
  }
  mdf[i, "Created"] <- split
...
mdf %>%
  select(Last.Updated, Created)
mdf <- mdf[names(mdf) != "Program.Contact.Email"]
for(i in 1:nrow(mdf)) {
  if(is.na(mdf[i, "FileName"])) {
    mdf[i, "file_size"] <- NA
  }
...
types <- c("csv", "json", "xls", "pdf", "zip", "tsv", "jpg", "jpeg", "png", "pdf", "txt")
for(i in 1:nrow(mdf)) {
  type <- ""
  for(j in types) {
    if(!is.na(mdf[i, "FileName"])) {
      if(str_detect(mdf[i, "FileName"], j)) {
        if(!is.na(mdf[i, "extension"])) {
          if(str_length(mdf[i, "extension"]) == 0) {
            if(str_length(type) == 0) {
              type <- str_c(type, j, sep = "")
            }else{
              type <- str_c(type, j, sep = ", ")
            }
          }
  ...
  mdf[i, "extension"] <- type
}
mdf[1:47, "extension"] <- "csv"
mdf[c("Last.Updated", "Created")] %>%
  filter(!is.na(Last.Updated) & !is.na(Created))
vec <- c()
for(i in 1:nrow(mdf)) {
  vec <- append(vec, mdf[i, "Last.Updated"] %>% mdy())
}
mdf$Last.Updated <- vec
vec <- c()
for(i in 1:nrow(mdf)) {
  if(is.na(mdf[i, "Created"])) {
    vec <- append(vec, mdf[i, "Last.Updated"])
  }else{
    vec <- append(vec, mdf[i, "Created"] %>% mdy())
  }
}
mdf$rel_age <- mdf$Last.Updated - mdf$Created
mdf$rel_age <- parse_number(as.character(mdf$rel_age))
ggplotly(ggplot(mdf) + geom_histogram(aes(x=rel_age)))
```
...
```{r}
curate("datahub", write = TRUE)
cron_ls()
vec_eq(mdf$X.1, c(1:nrow(mdf)))
vec_eq(col_to_vec(mdf["X.2"]), c(1:nrow(mdf)))
mdf <- drop_rowid(mdf)
mdf <- mdf[names(mdf) != "X"]
for(i in 1:nrow(mdf)) {
  if(!is.na(mdf[i, "Rights"]) & str_detect(mdf[i, "Rights"], "No ")) {
    mdf[i, "Rights"] <- "Unrestricted"
  }else{
    mdf[i, "Rights"] <- "Restricted"
  }
}
for(i in 1:nrow(mdf)) {
  if(mdf[i, "Rights"] == "Unrestricted") {
    mdf[i, "Public.Access.Level"] <- "Public"
  }else{
    mdf[i, "Public.Access.Level"] <- "Private/Restricted"
  }
}
```

```{r}
cdph$Repository <- "CDPH Open Data"
corgis$Repository <- "Corgis"
datagov$Repository <- "data.gov"
datahub$Repository <- "Datahub"
datashare$Repository <- "Edinburgh Datashare"
dryad$Repository <- "Data Dryad"
harvard$Repository <- "Harvard Dataverse"
re3data$Repository <- "re3data"
uci$Repository <- "UCI Machine Learning Repository"
zenodo$Repository <- "Zenodo"
full_repository <- c(cdph$Repository, corgis$Repository, datahub$Repository, datashare$Repository, dryad$Repository, harvard$Repository, re3data$Repository, uci$Repository)
mdf <- read.csv("./Data/mdf.csv")
for(i in 1:(nrow(mdf)-length(full_repository))) {
  full_repository <- append(full_repository, "Zenodo")
}
mdf$Repository <- full_repository
mdf %>%
  group_by(Name) %>%
  summarise(count = n()) %>%
  filter(count != 1)

scrape_cdph("https://data.ca.gov/dataset/west-nile-virus-cases-2006-present")
mdf2 <- read.csv("./Data/mdf.csv")
```
