...
full_df <- data.frame(matrix(ncol = ncol(test), nrow = 1))
...
for(i in 1:141) {
  ...
  links <- data.frame(links) %>%
    ...
    distinct()
  basic <- "https://data.ca/gov"
  for(j in links$links) {
    data_link <- paste(basic, j)
...
links <- html_attr(html_nodes(pg, "a"), "href")
links <- data.frame(links) %>%
  filter(str_detect(links, "/dataset/") & !identical(links, character(0))) %>%
  distinct()
#print(links$links)
for(j in links$links) {
  basic_link <- "https://data.ca.gov"
  data_link <- paste(basic_link, j, sep = "")
  print(data_link)
  full_df <- rbind(full_df, scrape_cdph(data_link))
}
full_df
